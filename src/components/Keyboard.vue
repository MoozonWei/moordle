...
const handleKeyClick = (e) => {
  ...
  // console.log(targetElement.innerText || 'BACKSPACE')
...
const whatColorIsThisKey = (keyStr) => {
  ...
  for (let i = 0; i < 6; i++) {
    for (let j = 0; j < 5; j++) {
      ...
      if (keyStr === store.boardLetters[i][j].toUpperCase() && store.boardColors[i][j] === 'h') return answer = 'font-white bgc-yellow'
      if (keyStr === store.boardLetters[i][j].toUpperCase() && store.boardColors[i][j] === 'f') return answer = 'font-white bgc-gray'
    }
  }
  return answer
}
